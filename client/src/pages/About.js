        ...
        <section className='animate animate__animated animate__fadeIn mxw-sm'>
          <div className='grid gap-6 grid-cols-1 sm:grid-cols-5'>
            <div className='sm:col-span-1'>
              <img
                className='h-full w-full 4 object-cover rounded-lg shrink-0'
                src={info?.avatar}
                alt=''
              />
            </div>
                ...
                {info?.socials &&
                  info?.socials.map((social) => (
                    <a
                      ...
                      rel='noreferrer'
                      href={social?.url}
                      className='inline-block my-1 bg-rose-500 text-white font-semibold py-2 px-3'
                    >
                      {social?.name}
                    </a>
                  ))}
        ...
        <section className='mxw-sm my-12'>
          ...
          <div className='flex items-center flex-wrap'>
            {about?.skills?.map((skill) => (
              <span className='py-2 px-3 my-1 mx-2 shadow-lg shadow-slate-300 bg-rose-500 text-white font-semibold'>
                {skill?.name}
              </span>
            ))}
          </div>
